#!/usr/bin/env python3
"""
Ultimate (Super) Tic-Tac-Toe - Pygame GUI Version with AI Opponent
...
# ---------- Backend: Ultimate Tic-Tac-Toe logic (with AI) ----------
...
class UltimateTicTacToe:
    def __init__(self):
        ...
        self.current = 1          # 1 = X (human), -1 = O (computer)
        self.next_sub = None      # (bi, bj) or None for free move
        self.game_over = False
        self.meta_winner = 0
        random.seed(42)           # for reproducible AI
    ...
    # ---------- AI Methods (computer plays as O = -1) ----------
    def _subboard_winning_move(self, board: np.ndarray, player: int):
    ...
    def _choose_move_in_subboard(self, bi, bj, computer=-1, human=1):
        """Choose a cell within a specific open sub-board using heuristic."""
    ...
    def computer_move(self):
        ...
        # 1. Can computer win a sub-board?
        for bi, bj in open_subs:
            board = self._subgrid(bi, bj)
            if self._subboard_winning_move(board, -1):
                ci, cj = self._choose_move_in_subboard(bi, bj)
                return (bi, bj, ci, cj)
        # 2. Block human from winning a sub-board?
        for bi, bj in open_subs:
            board = self._subgrid(bi, bj)
            if self._subboard_winning_move(board, 1):
                ci, cj = self._choose_move_in_subboard(bi, bj)
                return (bi, bj, ci, cj)
        # 3. Prefer center sub-board
        if (1, 1) in open_subs:
            bi, bj = (1, 1)
            ci, cj = self._choose_move_in_subboard(bi, bj)
            return (bi, bj, ci, cj)
        # 4. Prefer corner sub-boards
...
pygame.display.set_caption("Ultimate Tic-Tac-Toe - vs Computer")
...
def draw_board(game, highlight_sub=None):
    ...
    if game.game_over:
        if game.meta_winner == 1:
            ...
            status.append("Computer wins!")
        else:
            status.append("Draw!")
    else:
        turn = "You (X)" if game.current == 1 else "Computer (O)"
...
def main():
    ...
    # Small delay to let the player see the board before AI moves (if AI starts)
    ai_thinking_delay = 300  # milliseconds

    while running:
        draw_board(game, highlight_sub=game.next_sub)
        pygame.display.flip()

        # If it's computer's turn and game not over, make AI move
        if not game.game_over and game.current == -1:
            pygame.time.wait(ai_thinking_delay)  # subtle pause for realism
            move = game.computer_move()
            if move is not None:
                game.make_move(*move)
            else:
                # No legal moves? Game should be over, but just in case
                if not game.legal_moves():
                    game.game_over = True
            continue  # skip event handling this frame to avoid human input during AI turn

        # Handle events only when it's human's turn or game over
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN and not game.game_over and game.current == 1:
                pos = pygame.mouse.get_pos()
                move = get_cell_from_pos(pos)
                if move is not None:
                    bi, bj, ci, cj = move
                    game.make_move(bi, bj, ci, cj)
                    # The board will redraw and AI will move next iteration

        # If game over, show final board and wait for quit
        if game.game_over:
            draw_board(game, highlight_sub=None)
            if game.meta_winner == 1:
                msg = "You win! Click or press any key to quit."
            elif game.meta_winner == -1:
                msg = "Computer wins! Click or press any key to quit."
            else:
                msg = "Draw! Click or press any key to quit."
